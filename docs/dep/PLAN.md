...
| P16 | P4 零费用安全汇总 Gate 与 P12 handoff | [P16-harness-secret-egress-gate.md](plans/ongoing/P16-harness-secret-egress-gate.md) | P3 双网络 Squid、allow/deny/bypass、精确 model 绑定与真实 OpenCode Skill/MCP 本地 TLS Gate 已通过；下一步全仓/Compose 汇总，不启用 live |
...
P13、H0、P14 与 P15 已关闭。OpenCode `1.18.14` 已完成容器准入；R111-R118 完成 Receipt 落账、独立 Supervisor、产品拥有的 hash-locked Pack、真实 Skill/MCP/internal Mock，以及 PostgreSQL canonical Evidence → Candidate/API 本地 POC。普通 Compose 仍默认 replay；P12 是唯一产品执行主线。P16/P1-P3 已完成 `secret://` 合同、本地 tmpfs Store、通用 network policy、精确 Profile/provider/model/endpoint 绑定、双网络 Squid CONNECT gateway、allow/deny/bypass 与真实 OpenCode 本地 TLS 能力保持 Gate。当前进入 P4 汇总；显式 `harness` profile 可加载 DeepSeek runtime binding，但没有 secret/live 产品授权时不会调用。受控作用于 Attempt 外部副作用，不全局阉割 OpenCode 的 Skill/MCP/browser/工具循环；公共研究 recording gateway 仍是明确保留但尚未实现的独立能力。P16 完成后，P12 live 仍需用户另行提供获授权 ModelProfile、允许出站 Evidence、网络策略、单次预算与明确调用授权。真实供应商调用仍未授权、未发生。
