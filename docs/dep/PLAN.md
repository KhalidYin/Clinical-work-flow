...
| # | 当前 Gate | 子计划 | 状态 |
|---|----------|--------|------|
| P17 | P4 现有治理 UI 与全链路 Gate | [P17-knowledge-lifecycle-retrieval-poc.md](plans/ongoing/P17-knowledge-lifecycle-retrieval-poc.md) | P1-P3 done；P4 in progress，released Query Lab 与 Evaluation read workbench 已接通，不调用模型 |
...
P17 是当前唯一实施主线，也是 P12 未完成检索、评估与 Release 能力的聚焦实施合同，不建立平行产品权威。P12 live Gate 已冻结等待外部输入；P17/P1-P3 已完成 ICH E9 单文档离线 Recall、合成轮转/Evaluation、Release Worker 候选构建、人工发布、原子 current、历史重放和只读 REST/MCP manifest resolver；P4 已接通 current/历史 immutable Release FTS、真实 Query Lab 与 Evaluation read workbench，继续补 Evaluation 启动/候选重放、Releases/轮转治理 UI 和全链路 Gate，不依赖真实模型 Key。
