...
| # | 当前 Gate | 子计划 | 状态 |
|---|----------|--------|------|
| P12 | P2-B3 `secret://`/受控网络准备 → 单一 live vertical | [P12-knowledge-application-platform.md](plans/ongoing/P12-knowledge-application-platform.md) | P14 独立 Supervisor 离线 Gate done；live 仍需用户配置与单独授权，未调用 |
...
P13、H0 与 P14 已关闭。OpenCode `1.18.14` 已完成容器准入；R111-R114 完成 Receipt 落账、独立 Supervisor 的窄 HTTP 合同、机器身份、durable lifecycle、固定容器编译器、Knowledge remote provider，以及显式 Compose 私网/Worker 零 socket/真实离线 Attempt。普通 Compose 仍默认 replay；P12 是唯一执行主线，下一 Gate 不是自动 live，而是由用户另行提供获授权 ModelProfile、`secret://` 后端、允许出站 Evidence、网络策略与单次预算。真实供应商调用仍未授权、未发生。
