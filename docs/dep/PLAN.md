...
| # | 当前 Gate | 子计划 | 状态 |
|---|----------|--------|------|
| P12 | P2-B3 等待 P15 `secret://`/受控网络 Gate → 单一 live vertical | [P12-knowledge-application-platform.md](plans/ongoing/P12-knowledge-application-platform.md) | P15 已批准但未开始；live 仍需用户配置与单独授权，未调用 |

## 待开始

| # | 子计划 | 文件 | 预估轮次 | 依赖 |
|---|--------|------|----------|------|
| P15 | Harness 临时 Secret 与受控出站 Gate | [P15-harness-secret-egress-gate.md](plans/backlog/P15-harness-secret-egress-gate.md) | 6-9 | P14 完成；完成后恢复 P12 P2-B3 live Gate |

P13、H0 与 P14 已关闭。OpenCode `1.18.14` 已完成容器准入；R111-R114 完成 Receipt 落账、独立 Supervisor 的窄 HTTP 合同、机器身份、durable lifecycle、固定容器编译器、Knowledge remote provider，以及显式 Compose 私网/Worker 零 socket/真实离线 Attempt。普通 Compose 仍默认 replay；P12 是唯一产品执行主线，P15 是其已批准但未开始的安全前置 Gate，负责 Supervisor-owned tmpfs `secret://`、internal-only OpenCode 网络与 DeepSeek allowlisted egress proxy。P15 完成后，P12 live 仍需用户另行提供获授权 ModelProfile、允许出站 Evidence、网络策略、单次预算与明确调用授权。真实供应商调用仍未授权、未发生。
