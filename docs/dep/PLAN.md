...
| # | 当前 Gate | 子计划 | 状态 |
|---|----------|--------|------|
| P12 | P2-B3 等待 P15 Knowledge Harness POC → P16 `secret://`/受控网络 Gate → 单一 live vertical | [P12-knowledge-application-platform.md](plans/ongoing/P12-knowledge-application-platform.md) | P15 P1/P2 已完成，P3 与 P16 待执行；live 仍需用户配置与单独授权，未调用 |
| P15 | P3 Knowledge PostgreSQL Candidate/API 纵向 Gate | [P15-knowledge-opencode-harness-poc.md](plans/ongoing/P15-knowledge-opencode-harness-poc.md) | P1/P2 complete；固定 OpenCode Skill/MCP/internal Mock 成功和拒绝矩阵已通过 |
...
P13、H0 与 P14 已关闭。OpenCode `1.18.14` 已完成容器准入；R111-R114 完成 Receipt 落账、独立 Supervisor 的窄 HTTP 合同、机器身份、durable lifecycle、固定容器编译器、Knowledge remote provider，以及显式 Compose 私网/Worker 零 socket/真实离线 Attempt。普通 Compose 仍默认 replay；P12 是唯一产品执行主线。P15 P1 已冻结产品拥有的 hash-locked Harness Pack、Skill/MCP capability、配置隔离与 Supervisor 编译合同；P2 已完成真实 OpenCode、本地 Mock、Skill/MCP、零公网和拒绝矩阵，P3 再接 PostgreSQL Candidate/API 全链。P16 随后负责 Supervisor-owned tmpfs `secret://`、internal-only OpenCode 网络与 DeepSeek allowlisted egress proxy。P15/P16 完成后，P12 live 仍需用户另行提供获授权 ModelProfile、允许出站 Evidence、网络策略、单次预算与明确调用授权。真实供应商调用仍未授权、未发生。
