...
- [ ] `[UI-01]` 至 `[UI-04]` 的默认、loading、error、empty 状态可显示。
- [ ] React build 可由 FastAPI `/console/` 或新 `/workbench/` 路由服务。
- [ ] 前端不直接读取文件系统、不推断 run_state、不访问未声明 API。
- [ ] 行为测试覆盖首屏和 Run button 状态。
