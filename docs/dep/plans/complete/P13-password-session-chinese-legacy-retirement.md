...
- [x] `[UI-01]` 登录页保持 D0 视觉层级，用户名密码成功/失败/锁定行为符合合同，浏览器无法读取人员会话值。
- [x] `[UI-02]` 首次强制改密、主动改密、会话撤销、过期回登录页和目标 URL 恢复通过行为测试。
- [x] `[UI-03]` 产品名、九个一级导航、角色和退出均为中文，原有导航 URL 与键盘行为不变。
- [x] `[UI-04]` 各核心页面默认、加载、空、错误、部分数据和窄屏状态均完成中文视觉核验。
- [x] `[UI-05]` 管理员创建、重置、启停用户及一次性临时密码显示通过浏览器 E2E；越权请求被 API 拒绝。
- [x] `[UI-06]` 服务账号页面和前端网络响应不包含机器凭据值。
- [x] 所有设计偏差均已记录且为 `approved`。
- [x] 行为测试覆盖核心操作结果，不只检查标题或静态文本存在。
...
| P5 | 物理删除旧 Wiki 并完成全栈验收 | R019-R024 | P4 | done |
...
- [x] `[UI-01]`、`[UI-02]` 登录、强制改密、会话过期和目标 URL 恢复满足视觉与行为验收清单。
- [x] `[UI-03]`、`[UI-04]` 九个一级导航和核心页面完成中文化，技术标识/API 合同保持原值。
- [x] `[UI-05]`、`[UI-06]` 用户管理和服务账号安全边界通过组件测试与浏览器 E2E。
- [x] 默认、加载、空、错误、部分数据和窄屏状态完成截图/浏览器核验；无未批准视觉偏差。
- [x] 前端源代码和运行时存储不存在 `knowledgeLedgerBearerToken`、人员 token 输入框或 Authorization 注入。
...
- [x] `rg` 确认工作树不存在人员 bearer-token 登录、`sessionStorage` 凭据、旧 Vault/Obsidian/8787 运行依赖和 P1-P11 可执行计划指针。
- [x] 从空卷执行 compose build/migrate/bootstrap/start 成功；前端、API、PostgreSQL 和 Worker 健康。
- [x] 用户名密码登录、首次改密、退出、会话过期、管理员创建/重置/启停、越权与锁定 E2E 全部通过。
- [x] `[UI-01]` 至 `[UI-06]` 的视觉与行为清单、窄屏核验和中文文案检查全部通过。
- [x] 后端 unit/integration/migration、前端 unit/component/E2E、临床 Workflow 固定回归和零出站测试全部通过；真实外部模型 API 未被调用。
- [x] 每个阶段形成独立 Git 提交并同步远端；删除清单、迁移报告和最终提交可由 Git 审计与恢复。
...
| 2026-08-01 | P5 retirement / full-stack Gate | 精确删除 269 个旧运行资产；零引用、空卷 Compose、密码会话中文浏览器 E2E、全仓测试和 offline-replay 零出站 Gate 全部通过 |
