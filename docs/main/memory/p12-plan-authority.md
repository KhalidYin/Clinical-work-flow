...
**如何应用：** P1、P2-A 与 P2-B1 Gate 已关闭，下一任务是 P2-B2 fake/replay 可回放知识治理闭环。B2 只能复用已冻结的 fake/replay `ModelProviderPort`，从 `evidence_ready` 生成可回放 Candidate 并接通作者/Reviewer 页面行为；不能调用真实模型、建立生产索引或发布。P2-B3、P3、P4 仍需逐 Gate 推进。不得把 Evidence、Candidate 或 approved revision 当作 released knowledge，也不得反向修改 P1/P2-A/B1 已冻结的 Source、ObjectStore、权限、ledger、provenance 或四眼原则。如果未来需要重启 Workflow、Agent、Project Memory 或多 Study 协作，必须基于 P12 当时已发布的外部合同新建计划，不能恢复旧 P1-P11 文件继续执行。
