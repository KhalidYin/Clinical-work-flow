...
| Harness | 已有版本化合同、独立 Supervisor 生命周期、staging/MCP、OpenCode digest 准入、P15 PostgreSQL/API 本地 POC 及 P16/P2 本地 tmpfs `secret://`；普通 Compose 仍默认 replay | 以更收敛的生产 runtime/Secret authority 和受控出站部署 Harness Attempt，不向业务 Worker 暴露 Docker 控制权 |
...
P16/P1 已冻结能力保持型网络合同：产品只选择注册的 policy ID，Supervisor 在入队和 Executor 解析 secret 前分别 fail closed；`none` 是当前唯一 runtime-available 默认，`model-deepseek-v1` 仅是精确绑定的已定义策略，尚不能启动容器或出站。策略授权与 Agent capability 分离，因此获授权的 Skill/MCP/browser/工具循环不会被全局删除；同时 capability 也不能反向扩大 endpoint、数据边界或 secret。Receipt 只记录非敏感 policy/config/gateway 证据。

P16/P2 已实现本地 Supervisor-owned 临时 Secret：Compose 用独立 Docker local tmpfs volume 挂载 `/run/harness-secrets`，Worker 不挂载该卷；本机终端通过无回显 stdin 入口注入允许名称，`secret://deepseek-api-key` 由 Supervisor 解析，并为每个 Attempt 生成不可从路径反推 ID 的只读 OpenCode auth 文件。成功、失败、timeout、cancel、orphan、写入异常和 Supervisor 重启均有清理/丢失语义，清理失败会留下脱敏失败证据并阻止成功结果。tmpfs volume 与持久 state volume 使用不同 daemon path mapper，避免认证材料写入 durable journal/workspace。它仍只是本地临时 Store，不是 Vault/云 Secret Manager；P3 egress gateway、DeepSeek live 和生产 runtime authority 尚未完成。
