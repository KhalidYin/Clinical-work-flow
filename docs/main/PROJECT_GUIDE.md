...
| 领域 | 当前基线 | 目标状态 |
|------|----------|----------|
| 知识控制面 | PostgreSQL durable DAG、Document/Enrichment/Release Worker、ObjectStore、Candidate/Review、React GUI 已有骨架；P17/P1-P3 已接通版本化 Chunk、轮转、E9 Recall、Evaluation 与 immutable Release/current/history/manifest resolver，P4 已接 Release Query Lab、Evaluation、Releases、只读 Chunk Inspector 与 Rotation Queue | 补全 SourceVersion 比较、生命周期谱系/审计、vector/relation route 与生产级 Knowledge MCP 接线 |
...
| GUI | 九个一级导航和核心治理页面已有框架；Query Lab、Evaluation、Releases 已接真实 API，其余治理页面继续增量完善 | 细化为 Workflow 观察、人工治理、评估和发布控制台；聊天不是治理入口 |
...
index 时相应 capability 明确 degraded，generation disabled，且不填充假的 score。官方 E9 仅保存在本地
ignored `.poc-assets/`，18 条原创 GoldCase 基线得到 Recall@5 `0.888889`、Recall@10 `0.944444`，报告明确
不是临床质量认证。该 POC 未创建 Candidate 或 current Release；Chunk 仍是可重建投影，不是第三套权威。
...
并与 `passed/failed` 的合成 Release Gate 用途严格区分。服务端登记固定 suite；浏览器启动只提交 suite
身份，不提交 SourceVersion/ChunkProfile。失败案例以 EvaluationRun/Case 身份重放，服务端从不可变 run
恢复候选范围；同 suite/purpose 的回归差异也由服务端计算。损坏记录产生 partial warning 或完整性错误，
不回退到报告文件。默认 E9 命令使用临时数据库，报告明确 `database_retention=ephemeral`；它证明持久化
合同，但不会给正在运行的 Compose 数据库留下记录。
...
canonical 状态；Rotation Queue 只显示服务端 `allowedActions`，proposal/decision 携带 case version 与幂等键，
stale 后刷新 canonical Case，并展示不可变 DecisionReceipt。SourceVersion 比较页面、生命周期谱系/审计 read model
和认证真实浏览器/390px 验收尚未完成。
...
首期候选已选定 OpenCode `1.18.14`。知识侧 `opencode-supervised` 已成为 remote provider：Worker 只提交产品级、hash-locked Attempt；独立 Supervisor 拥有 Bearer 机器身份、durable journal、heartbeat/cancel/orphan recovery、固定编译器和终态幂等。显式 Compose `harness` profile 使用内部 control network，仅 Supervisor 持有宿主 Docker socket，并把容器内 state volume 路径映射为 daemon-visible bind source。真实合成凭据 Attempt 证明子容器固定 digest、`network none`、非 root、只读 rootfs、cap-drop ALL、no-new-privileges 和资源上限，失败后容器、workspace 与临时 secret 清理，Receipt 可重复查询。该 Gate 仍是本地离线部署，不是 live/生产出站：普通 Compose 继续 replay，受控网络和更收敛的 runtime authority 仍待完成。多 Harness 路由、多 Agent 协作和跨租户调度不属于当前阶段。

P16/P1 已冻结能力保持型网络合同：产品只选择注册的 policy ID，Supervisor 在入队和 Executor 解析 secret 前分别 fail closed；`model-deepseek-v1` 精确绑定 Profile、provider、model、endpoint、data boundary 和 secret name。策略授权与 Agent capability 分离，因此获授权的 Skill/MCP/browser/工具循环不会被全局删除；同时 capability 也不能反向扩大 endpoint、数据边界或 secret。Receipt 只记录非敏感 policy/config/gateway 证据。

P16/P2 已实现本地 Supervisor-owned 临时 Secret：Compose 用独立 Docker local tmpfs volume 挂载 `/run/harness-secrets`，Worker 不挂载该卷；本机终端通过无回显 stdin 入口注入允许名称，`secret://deepseek-api-key` 由 Supervisor 解析，并为每个 Attempt 生成不可从路径反推 ID 的只读 OpenCode auth 文件。成功、失败、timeout、cancel、orphan、写入异常和 Supervisor 重启均有清理/丢失语义，清理失败会留下脱敏失败证据并阻止成功结果。tmpfs volume 与持久 state volume 使用不同 daemon path mapper，避免认证材料写入 durable journal/workspace。它仍只是本地临时 Store，不是 Vault/云 Secret Manager。

P16/P3-P4 已实现并验收首个能力保持型模型出站实例：OpenCode 只连接 `internal: true` client network，digest/hash-locked Squid gateway 是唯一双宿主服务，并只允许 CONNECT `api.deepseek.com:443`；其他 hostname、IP、端口、私网/保留地址及无代理直连失败。Squid 不做 TLS 解密，gateway 不持有 key。真实固定 OpenCode 已通过本地 TLS 假端点完成 Pack Skill、`read_evidence` MCP 与多轮模型工具调用；全仓、空卷 migration、Compose 与离线 Smoke Gate 通过。显式 `harness` profile 启动双宿主 gateway，扩大了本地部署面；普通 Compose 仍 replay，Worker/Supervisor 不连接 client/uplink。生产 socket/Secret authority、公共研究 recording gateway 和 DeepSeek live 仍未完成。

P15 已新增产品拥有的 `clinical-llm-wiki/harness-packs/knowledge-candidate-v1/`：Pack 只声明 instruction、Skill、schema 与逻辑 MCP capability，Supervisor 在每个 Attempt 编译 `/workspace/.opencode/skills`、隔离 HOME/XDG、默认 deny permission、`read_evidence` MCP 和内部 OpenAI-compatible provider。固定 OpenCode 真实走 `/v1/responses`，从 PostgreSQL canonical Evidence 生成 schema-valid Candidate；产品 Validator 写入唯一 ModelInvocation/Candidate，API 核对 Evidence 与 invocation lineage，状态停在 `author_confirmation_required`。签入的本地 POC loop 每次从随机空卷项目运行首次 Worker和重复 Worker，再从 PostgreSQL、Execution/Validation Receipt、正式 Cookie API 和 internal Mock audit 交叉证明唯一 lineage、重复零增量、`network_policy=none` 与 DeepSeek 零请求；loop JSON 只是测试报告，不是第三套状态。主模型与 `small_model` 同锁，合成 key 通过独立只读文件进入 `env://` resolver；它不是生产 Secret Store。项目/外部 Skill 被隔离，但镜像内建 `customize-opencode` 仍存在；Docker internal 网络、socket authority、Attempt staging bind 和 Linux 临时目录宽写权限都只是本地 POC 证据，不是生产 Secret、egress、模型质量或 runtime authority 认证。
