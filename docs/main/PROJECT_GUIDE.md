...
P16/P3 已实现首个能力保持型模型出站实例：OpenCode 只连接 `internal: true` client network，digest/hash-locked Squid gateway 是唯一双宿主服务，并只允许 CONNECT `api.deepseek.com:443`；其他 hostname、IP、端口、私网/保留地址及无代理直连失败。Squid 不做 TLS 解密，gateway 不持有 key。真实固定 OpenCode 已通过本地 TLS 假端点完成 Pack Skill、`read_evidence` MCP 与多轮模型工具调用，证明网络受控不等于删除 Agent 能力。显式 `harness` profile 启动双宿主 gateway，扩大了本地部署面；普通 Compose 仍 replay，Worker/Supervisor 不连接 client/uplink。P4 全量 Gate、生产 socket/Secret authority、公共研究 recording gateway 和 DeepSeek live 仍未完成。
