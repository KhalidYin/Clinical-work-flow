...
- 容器化成熟 Harness：共享执行基础设施，不是第三个产品；H0、OpenCode `1.18.14` 容器准入、独立 Supervisor、P15 Knowledge PostgreSQL/API 本地 POC 及 P16/P2-P3 本地临时 `secret://`/模型 endpoint gateway 已完成；P16/P4、生产 Secret/runtime authority、公共研究网关和 DeepSeek live 尚未完成。
...
当前签入的 Study 只有 draft runtime manifest，不应直接用 `agent_loop` 当作可运行产品入口；它仍是迁移输入，且默认可自动创建目录和 Git commit。受控示意用法与前置条件见 `USAGE.md`。P15/P16 OpenCode 本地 POC 不等于生产 Runtime；P16/P2-P3 仅完成本地 `secret://` tmpfs Store 与模型 endpoint gateway，P16/P4、生产 Secret/runtime authority、公共研究网关、DeepSeek live 和临床 Workflow Harness 化尚未完成。
