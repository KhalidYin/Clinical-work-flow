---
id: moc-home
type: navigation
title: Clinical LLM Wiki 首页
---

# Clinical LLM Wiki

这里维护可审计的临床统计工作流知识。请选择一种进入方式；四条路径访问的是同一批 Markdown 正文。

## 按角色

- **临床统计师 / 统计程序员**：[[10_MOC/Workflow-MOC|工作流 MOC]]、[[10_MOC/Knowledge-MOC|领域知识 MOC]]。
- **审核人**：[[80_Governance/README|治理、审核与批准证据]]。
- **知识维护人**：[[98_Inbox/README|收件箱]]、[[90_System/Templates/README|模板]]、[[60_Sources/Registry/README|来源登记]]。
- **Study 负责人**：[[70_Prior_Studies/README|既往研究模式]]；当前 Study 规则仍必须留在 Study 工作区，不能写回这里冒充通用规则。

## 按十阶段

[[30_Workflows/Stages/Protocol Analysis|1. Protocol Analysis]] · [[30_Workflows/Stages/SAP Generation|2. SAP Generation]] · [[30_Workflows/Stages/SDTM Spec Baseline|3. SDTM Spec]] · [[30_Workflows/Stages/SDTM Programming|4. SDTM Programming]] · [[30_Workflows/Stages/ADaM Spec|5. ADaM Spec]] · [[30_Workflows/Stages/ADaM Programming|6. ADaM Programming]] · [[30_Workflows/Stages/TFL Shell Design|7. TFL Shell Design]] · [[30_Workflows/Stages/TFL Programming|8. TFL Programming]] · [[30_Workflows/Stages/QC Validation|9. QC Validation]] · [[30_Workflows/Stages/Submission Packaging|10. Submission Packaging]]

## P5 引导路径

- [[10_MOC/Methods-MOC|方法选择与假设]]
- [[10_MOC/Standards-MOC|标准与法规边界]]
- [[10_MOC/Programming-MOC|编程模式与验证等级]]
- [[10_MOC/Stage-Traceability-MOC|端到端追溯链]]
- [[10_MOC/Toolkit-MOC|QC 检查表、问题处置与证据包]]
- [[10_MOC/Sources-MOC|来源、页码与视觉证据]]
- [[10_MOC/Cases-MOC|合成案例与沉淀候选]]

## 按知识域

- [[20_Knowledge/Concepts/README|概念]]
- [[20_Knowledge/Methods/README|统计方法]]
- [[20_Knowledge/Standards/README|标准与法规]]
- [[20_Knowledge/Decisions/README|决策规则]]
- [[20_Knowledge/Programming/README|编程模式]]

## 按工具与交付物

- [[40_Toolkit/Checklists/README|检查表]]
- [[40_Toolkit/Deliverable-Patterns/README|交付物模式]]
- [[40_Toolkit/Decision-Trees/README|决策树]]
- [[60_Sources/README|来源与图表证据]]

## 系统约束

运行时只消费经审核的结构化上下文；自由问答、Obsidian 插件输出和未批准草稿均不能作为生产依据。详见 [[80_Governance/README|治理入口]]。
