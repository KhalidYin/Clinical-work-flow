...
这是由 P12/P13 建立的可运行知识应用骨架：React 前端、FastAPI、PostgreSQL/pgvector、拒绝覆盖写并校验 hash 的对象存储和独立异步 Worker。Source → Evidence → Candidate → 人工治理主链路已具备；P17/P2 已接通 release-candidate Query Lab 后端和 ICH E9 离线 Recall 报告，但 Evaluation/Release 治理页面、通用 Release Builder 与标准知识 MCP 尚未形成完整闭环。
...
在本目录运行 `python -m scripts.ich_e9_poc` 可下载或复用 hash 固定的官方 E9，启动临时 pgvector、执行 Document→Chunk→metadata/FTS→18 条 GoldCase，并生成 `reports/p17/ich-e9-retrieval-baseline.json` 后自动清理容器。原始 PDF 位于 ignored `.poc-assets/`，该命令不需要模型 Key，也不会发布 current Release。
