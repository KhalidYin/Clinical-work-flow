...
class VaultRepository:
    ...
    def _write_proposal_packet(self, record_id: str, path: Path, record: dict[str, Any]) -> None:
        ...
        packet = {
            ...
            "agent_summary": f"知识候选 {record_id} 需要完成治理审核后才能使用。",
            "findings": [{
                "id": "F-001", "category": "compliance", "severity": "warning",
                "location": record_id, "title": "批准受治理的知识候选",
                "current_value": "待审核候选", "proposed_value": record["title"],
                "rationale": "进入生产使用范围前必须取得人工 DecisionReceipt。",
