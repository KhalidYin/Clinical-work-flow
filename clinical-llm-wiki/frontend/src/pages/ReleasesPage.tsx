...
export function ReleasesPage({ candidateId, onCandidateChange }: ReleasesPageProps) {
            ...
            {record.history.length ? (
              <ol className={styles.releaseHistory}>
                {record.history.map((item) => (
                  <li key={item.releaseId}><strong>{item.version}</strong><code>{item.releaseId}</code><span>{item.itemCount} revisions</span></li>
                ))}
              </ol>
